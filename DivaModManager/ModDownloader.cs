    ...
    public class ModDownloader
    {
        ...
        private void ExtractFile(string fileName, string game, GameBananaRecord record)
        {
            ...
            if (File.Exists(_ArchiveSource))
            {
                try
                {
                    if (Path.GetExtension(_ArchiveSource).Equals(".zip", StringComparison.InvariantCultureIgnoreCase))
                    {
                        var psi = new System.Diagnostics.ProcessStartInfo
                        {
                            FileName = "tar.exe",
                            Arguments = $"-xf \"{_ArchiveSource}\" -C \"{ArchiveDestination}\"",
                        ...
                        using (var proc = System.Diagnostics.Process.Start(psi))
                        {
                            ...
                            if (proc.ExitCode != 0)
                            {
                                ...
                                throw new Exception($"tar.exe fallo (codigo {proc.ExitCode}): {err}");
                            }
                        }
                    }
                    else if (Path.GetExtension(_ArchiveSource).Equals(".7z", StringComparison.InvariantCultureIgnoreCase))
                    {
